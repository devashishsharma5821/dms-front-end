...
const Details = (props: DetailsPropsType) => {
    const [TransformersData, ExperimentData, moduleConfigData, experimentToSave, graph, stages, UserConfig, experimentSelectedCompute] = useAppStore((state: DetailsAppStoreState) => [
        ...
        state.stages,
        state.UserConfig,
        state.experimentSelectedCompute
    ]);
    const opid = v4();
    ...
    const runInference = (formData: any) => {
        console.log('lets check runInference working or not onClick ===>', formData);
        if (props?.selectedStageId) {
            ...
            if (formData && Object.keys(formData).length > 0 && currentSelectedStage.id && graph && stages && experimentId && UserConfig.userConfiguration.user.userId && experimentSelectedCompute) {
                console.log('lets check going inside that if check or not ==>', experimentSelectedCompute);
                if (projectId) {
                    let msg = BusHelper.GetInferOutputRunRequestMessage(
                        {
                            ...
                            userId: '355',
                            // userId: experimentSelectedCompute.id,
        ...
        <>
            <Drawer isOpen={props?.isOpen} placement="right" onClose={props?.onClose} colorScheme={useColorModeValue('light.whiteText', 'dark.veryDarkGrayishBlue')}>
                ...
                <DrawerContent mt="64px" bg={useColorModeValue('default.whiteText', 'dark.veryDarkGrayishBlue')}>
                    <DrawerCloseButton />
                    <DrawerBody mt="30px">
                        <Form schema={JSON.parse(transformer?.schema?.jsonSchema)} formData={formData} onChange={handleChange} onSubmit={onSubmitHandler} omitExtraData={true} validator={validator} />
                    </DrawerBody>
                    {/* 
                    <DrawerFooter>
