...
const Designer = (props: any) => {
    ...
    React.useEffect(() => {
        ...
        return () => {
            window.removeEventListener('resize', setPaperDimensions);
        };
    ...
    return (
        <Box position="relative" background={canvasBackground} backgroundSize={canvasBackgroundSize}>
            ...
            <Box ref={designerElement} className="designer">
                <Wrap className="canvas" ref={canvas} />
            </Box>
            <Box position="absolute" bottom="var(--chakra-space-10)">
                <Wrap id="navigator" overflow="unset" />
            </Box>
        </Box>
    );
};
